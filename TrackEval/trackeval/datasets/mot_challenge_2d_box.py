...
class MotChallenge2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        self.tracker_sub_fol = self.config['TRACKER_SUB_FOLDER'] if self.config['TRACKER_SUB_FOLDER'] is not None else ""
    ...
    def _get_seq_info(self):
        ...
        seq_list = [
            d for d in os.listdir(self.gt_fol)
            if os.path.isdir(os.path.join(self.gt_fol, d))
        ]
        
        seq_lengths = {}

        for seq in seq_list:
            img_dir = os.path.join(self.gt_fol, seq, "img")
